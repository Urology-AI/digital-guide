...
Petrie Department of Urology. See `docs/SHOWCASE.md` for the demo script and
review checklists.

### How it is put together

```
src/data/care/      typed clinical content, citation registry, retrieval corpus,
                    intent gates, red flags — the clinical source of truth
src/features/care/  design tokens and primitives, routes, pages, Copilot
src/components/guide/  the full referenced clinical guide (#/guide)
src/i18n/           interface strings (en/es/hi; clinical content is English)
```

Content is authored as typed TypeScript, never hand-maintained JSON, so the
compiler and the test suite both validate it. The retrieval corpus is projected
from that content at runtime — retrieval can only ever surface approved text.

### The safety model

Three checks run in order before a patient sees anything:

1. **Red flags** — symptoms needing care now are answered with urgent guidance
   before any educational content.
2. **Intent gates** — diagnosis, interpretation of an individual result,
   treatment selection, prognosis, medication changes and post-procedure
   activity never reach retrieval. Each abstains with a care-team handoff.
3. **Retrieval** — passages are returned verbatim with citations, restricted to
   content matching the question's procedural context. Below threshold it
   abstains rather than offering a weak match.

Nothing is generated by a language model, so the product cannot state something
the approved content does not say.
...
npx tsc --noEmit
npm test
npm run build -- --base="/digital-guide/"
```

The test suite fails the build if a content item loses its citation, if a
citation points at a missing registry entry, if absolute clinical language
appears, or if any query in the safety matrix stops behaving correctly.
